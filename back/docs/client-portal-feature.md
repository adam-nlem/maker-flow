...
| `GET /api/post-drafts`, `GET /api/post-drafts/{uuid}`, `GET /api/post-draft-media-versions/files`, `GET /api/post-draft-media-versions/stream` | `ProjectVoter::VIEW` | Read-only. Clients see only the drafts of their own project. |
| `POST /api/post-draft-media-versions/{mediaVersionUuid}/approve`, `POST /api/post-draft-media-versions/{mediaVersionUuid}/request-changes` | `ProjectVoter::VIEW` + `IsGranted(ROLE_CLIENT)` | **Client-only** review actions (agency Admin/Editor are explicitly excluded from self-approval). See `back/docs/post-draft-feature.md` for the per-version status state machine and the comment thread model. |
| `POST /api/post-draft-media-versions/{mediaVersionUuid}/comments` | `ProjectVoter::VIEW` | Open to both agency members and clients — the per-version comment thread is bidirectional. Comment only, no status change. |

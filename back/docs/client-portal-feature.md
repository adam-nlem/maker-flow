...
| `POST /api/review-versions/{reviewVersionUuid}/approve`, `POST /api/review-versions/{reviewVersionUuid}/request-changes` | `ProjectVoter::VIEW` + `IsGranted(ROLE_CLIENT)` | **Client-only** review actions (agency Admin/Editor are explicitly excluded from self-approval). See `back/docs/review-feature.md` for the per-version status state machine and the comment thread model. |

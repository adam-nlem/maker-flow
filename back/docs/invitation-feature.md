...
- `ClientWelcomeEmailTemplate` — subject `Bienvenue sur votre portail {agencyName}`, body uses the fixed brand color `#141115` for the heading and CTA; includes the agency's `contactEmail` when present
...
- `api_invitation_show` — used by the public summary endpoint (`GET /api/invitations/{token}`). Tagged on `Invitation` (uuid, type, email, firstName, lastName, role, expiresAt, createdAt, agency, project, createdBy), `Agency.name`, `Project.name`, `User.firstName` + `User.lastName` (so the `createdBy` sub-object only exposes a display name).
...
| Endpoint | Method | Route Name | Auth | Description |
|----------|--------|------------|------|-------------|
| `/api/agencies` | POST | `api_agencies_create` | `ROLE_ADMIN` | Onboarding step: admin creates their agency. Body: `{name, contactEmail?, website?}`. 409 `27002` if the user already has one. |
...
3. `POST /api/agencies` `{name, contactEmail?, website?}` → agency persisted, user linked, welcome credits granted, returned via `api_agency_create`.
...
2. Email goes out with the fixed MakerFlow brand color

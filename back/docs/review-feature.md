...
| `GET /api/reviews/{uuid}` | `User` (auth only) | `PROJECT_VIEW` on the draft's project | Detail. Returns the Review with a top-level `latestVersion` field (full version data, **no comments**). Older versions are not exposed. Same gate rationale as the list. |
...
- `ReviewWithLatestVersionResponseDTO` — used by **every** Review-shaped endpoint: `GET /api/reviews` (list), `GET /api/reviews/{uuid}` (show), create / update, approve / request-changes, comment-create / comment-update. Constructor-injected `{review, latestVersion}`, with the wrapper tagged with **all** the groups the project uses so the same DTO serializes correctly under whichever context the controller passes. Per the project convention the group name matches the route name — each endpoint passes its own group; the alternates are listed alongside `api_reviews_show` on the relevant `Review`, `ReviewVersion`, and `Script` properties + on `ReviewWithLatestVersionResponseDTO`. Field selection is driven by the **entity-level** `#[Groups]` on `Review` / `ReviewVersion` / `Script`:
  - `api_reviews_list` (used by `GET /api/reviews`) → `review.{uuid, title, mediaType, createdAt}` + `latestVersion.{uuid, fileCount, status, videoStreamingStatus, videoStreamingFailureReason}` (no `latestVersion.createdAt`, no `comments`, no `script`, no `description` / `notes` / `updatedAt`).
  - `api_reviews_show` (used by `GET /api/reviews/{uuid}`) → adds `review.{description, notes, updatedAt, script}` and `latestVersion.createdAt`. `script` is the `Script` entity exposed as `uuid + title`.
  - `api_reviews_create` (create), `api_reviews_update` (update), `api_review_versions_approve` (approve), `api_review_versions_request_changes` (request-changes), `api_review_comments_create` (comment-create), `api_review_comments_update` (comment-update) → same full field set as `api_reviews_show`.

...
- **Response shape includes nested `agency`:** `uuid`, `name`, `description`, `types`, `createdAt`, `updatedAt`, `finishedAt`, and `agency: { uuid, name, accentColor, backgroundColor, backgroundSecondaryColor, textColor, textSecondaryColor, headingFont, bodyFont, contactEmail, website }`. List endpoints (`api_projects_get_paginated`) do **not** include the agency — only this get-by-uuid response does.
...
| `api_project_get_by_uuid` | Show endpoint response | **Includes nested `agency` with the full theming payload (uuid, name, accentColor, backgroundColor, backgroundSecondaryColor, textColor, textSecondaryColor, headingFont, bodyFont, contactEmail, website)** so the client portal can theme itself without a separate request |

...
export function getMaxScriptsPerProjectForPlan(plan: SubscriptionPlan | null | undefined): number | null {
    if (!plan) return 1;
    return planConfigs.find((c) => c.plan === plan)?.maxScriptsPerProject ?? null;
}

...
| `components/client-portal/sidebar/ClientDesktopSidebar.tsx` | Client sidebar content — agency-branded header (name tinted with `agency.brandColor`), Home + Contents nav, a PLATEFORMES section with per-platform `IntegrationTile` rows (same as the agency sidebar), and Settings at the bottom. Fetches the agency via `useShowProject(user.clientProjectUuid)`. Mounts `IntegrationLoginModal` once for the whole client shell. Composed inside `SidebarShell`. |

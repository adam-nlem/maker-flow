...
export default function ScriptCalendarDayCell({ droppableId, day, isToday, scripts, onScriptClick, onCreateScript }: ScriptCalendarDayCellProps) {
    ...
    return (
        <div ref={setNodeRef} className={`group relative flex flex-col border-b border-r border-pale-gray p-1.5 transition-colors overflow-hidden ${isOver ? "bg-primary/5" : ""}`}>
            <div className="flex flex-row items-center justify-between mb-1">
                {isToday ? (
                    ...
                    <span className="text-heading-xs text-muted-2 px-1">{day}</span>
                )}

                <button
                    onClick={onCreateScript}
                    className="opacity-0 group-hover:opacity-100 transition-opacity text-muted-2 hover:text-primary cursor-pointer"
                >
